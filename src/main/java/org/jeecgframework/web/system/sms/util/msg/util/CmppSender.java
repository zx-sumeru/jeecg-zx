	...
	public void start() throws Exception {
		if (out != null && null != sendData) {
			for (byte[] data : sendData) {
				log.info("发送的二进制队列里data长度====" + data.length);
				sendMsg(data);
				byte[] returnData = getInData();
				log.info("发送的二进制队列里响应值的长度====" + returnData.length);
				if (null != returnData) {
